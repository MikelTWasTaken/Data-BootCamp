...
-- Lets get all the dates that accounts were created. Write a query to 
...
-- FROM film
-- ORDER BY release_year ASC, rental_rate ASC;

-- Write a query to get the address, and the phone number 
-- of all customers living in the Texas district, these details can be found in the “address” table.
...
-- Write a query which should check if your favorite movie exists 
-- in the database. Have your query get the film ID, title, description, 
...
-- title, description, length and the rental rate of all the movies starting with the two first letters 
...
-- Write a query which will join the data in the customer table and the payment table. You want to get the 
-- first name and last name from the curstomer table, as well as the amount and the date of every 
